...
- **搜索与 AI 提问**：支持 Google、百度、Bing、搜狗和 360 搜索，中文输入法组合拼音期间也会实时请求联想；还可把纯文字问题直接转发到豆包或 DeepSeek 网页端。
- **灵活的快捷方式**：添加、编辑、批量删除、跨页移动、拖动排序和分组；支持隐藏或设置 1–3 行上限，并用分页圆点快速跳转。
- **清晰的站点图标**：内置资源使用随扩展打包的本地图标，其他网站并行检查高清图标、第三方图标源和浏览器 favicon，并尽量把结果持久化到本地缓存。
...
- **隐私说明可见**：设置中直接说明本地数据、搜索/AI 直连和可选第三方请求，并提供完整隐私政策入口。
...
- 点击彩色搜索引擎名称可切换 Google、百度、Bing、搜狗或 360 搜索。
- 输入关键词后直接搜索；输入网址则直接访问。
- 支持搜索建议、语音搜索入口、图片搜索入口和 AI 搜索入口。
- 选择豆包或 DeepSeek 后，可使用与聊天网站一致的多行输入框：Enter 发送、Shift+Enter 换行，并可选择“深度思考”和“联网搜索”。扩展会打开对应网页、应用可识别的选项、填写并发送问题；待网页确认发送后才清除会话任务，登录或页面加载较慢时不会提前丢失。
...
- 自动探测 SVG、Apple Touch Icon、高清 PNG、浏览器 favicon 和备用图标服务，优先使用清晰度更高的来源。已解析来源会缓存 30 天；满足跨域与大小条件的图片会直接保存为本地数据，其他图片继续使用浏览器 HTTP/favicon 缓存。
