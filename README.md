...
- **搜索与 AI 提问**：支持 Google、百度、Bing、搜狗和 360 搜索，也可把纯文字问题直接转发到豆包或 DeepSeek 网页端。
...
- 选择豆包或 DeepSeek 后，输入纯文字问题并按 Enter，扩展会打开对应网页、填写问题并尝试点击发送。AI 问题仅在当前浏览器会话内短暂传递，不加入 LumoraTab 搜索历史；登录或网站自身原因造成的失败不会重试。
...
| 豆包与 DeepSeek 网页访问 | 在用户主动提问后填写纯文字问题并尝试点击发送 |
